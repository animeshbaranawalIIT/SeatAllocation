...
public class GaleShapleyAdmission {
	...
	/**
	 * The constructor of this class takes three filenames as inputs 
	...
	public GaleShapleyAdmission(String f1,String f2,String f3){
		...
		for(int i=0;i<implist.retfinlist().size();i++){
			...
			// Candidates which get no seat and are of GE/SC/ST rank are eligible for phase2
			if(x.retwaitlisted().equals("-1") && (ranklist.get(x.retID())[0]!=0 || ranklist.get(x.retID())[2]!=0 || ranklist.get(x.retID())[3]!=0)){ 
				x.setp2pref(ranklist.get(x.retID())); // initialise modified preferences
				x.setcurrent(x.retp2pref().get(0)); // initialise current to the start of modified preference 
				x.setphase2(true); // set phase2 status of candidate to true
			}
			// Candidates for which this is not true are left as it is .... removed from the merit list
			else { temp.add(x); }
		} /// this modifies the merit list
		for(int i=0;i<temp.size();i++){ implist.removefin(temp.get(i)); }
		
		// if the size of new merit list > 0
		// implies phase2 required to run
		if(implist.retfinlist().size()>0){
			this.implist.resetmerit(ranklist); //modify the merit list for phase2
			for(int i=0;i<proglist.size();i++){ proglist.get(i).prepphase2(); } //prepare each program for phase2
			this.setmeritprogs(); // re distribute the new merit lists to the programmes 
			this.phase2(); // undertake phase2 of program
		}
	 ...
	 public void fillcandidatedata(String f2){
		...
		try{
			...
			while((line1=reader1.readLine())!=null){
				...
				for(int i=0;i<temp2.length;i++) x.add_preference(ranklist.get(temp1[0]), temp2[i]); // initialise preference list of candidate
				x.setcurrent(x.retpreference().get(0)); //set current data member of candidate to his first choice in preference list
				
				// if candidate is of foreign category and has general rank, gets his first preferred choice
	...
	/** 
	 * DERESERVATION <br>
	 * this function allots seats to candidates which are in modified merit list<br>
	 * Candidate checked if he has been waitlisted for some program or
	 * whether his modified preference list been exhausted<br>
	 * If none applicable for any candidate then candidate applies to next program on his
	 * preference list <br>
	 * if this is applicable for every candidate algorithm terminates
	 */
	public void phase2(){
		...
		while(!complete){
			...
			for(int i=0;i<proglist.size();i++){
				proglist.get(i).filter2(); // filtering applicants by each prog but with modified meritlist
			}
		}
	}
}
